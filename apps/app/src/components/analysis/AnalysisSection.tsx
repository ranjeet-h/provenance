...
export function AnalysisSection({
  ...
  const [reportingStudentId, setReportingStudentId] = React.useState<
    string | null
  >(null);
  const [reportError, setReportError] = React.useState<string | null>(null);
  const [reportNotice, setReportNotice] = React.useState<string | null>(null);
  const [selfCheckStudentId, setSelfCheckStudentId] = React.useState<
    string | null
  >(null);
  const [selfCheckError, setSelfCheckError] = React.useState<string | null>(
    null,
  );
  const [selfCheckNotice, setSelfCheckNotice] = React.useState<string | null>(
    null,
  );
  ...
  const [verificationError, setVerificationError] = React.useState<
    string | null
  >(null);
  const [verificationNote, setVerificationNote] = React.useState<string | null>(
    null,
  );
  ...
  const canAnalyze =
    submissionCount >= 2 ||
    (submissionCount >= 1 && selectedLibraryIds.length > 0);
  const displayError =
    error ??
  ...
  async function onGenerateReport(studentId: string): Promise<void> {
    ...
    try {
      ...
      const savedPath = await saveReportFiles(
        exported.pdfBytes,
        `provenance-${studentId}-certified-report.pdf`,
        exported.signedReportJson,
      );
      setReportNotice(
        savedPath === null
          ? "Save cancelled; no report files were written."
          : `Certified PDF saved to ${savedPath}; verification JSON saved beside it.`,
      );
    } catch (cause) {
  ...
  async function onGenerateSelfCheck(studentId: string): Promise<void> {
    ...
    try {
      ...
      const savedPath = await saveReportFiles(
        bytes,
        `provenance-${studentId}-self-check.pdf`,
      );
      setSelfCheckNotice(
        savedPath === null
          ? "Save cancelled; no PDF was written."
          : `Self Check PDF saved to ${savedPath}. It is not teacher certified.`,
      );
    } catch (cause) {
  ...
  async function onVerifyReport(): Promise<void> {
    ...
    try {
      if (verificationFile.size > 20_000_000) {
        throw {
          code: "validation",
          message: "Choose a signed report JSON file under 20 MB.",
        };
      }
      const result = await verifyCertifiedStudentReport(
        await verificationFile.text(),
      );
      setVerificationNote(
        `Signature verified · key ${result.signing_key_id}. ${result.note}`,
      );
    } catch (cause) {
  ...
  const selectedPair =
    report?.pairs.find((p) => pairKey(p) === selected) ?? null;

  return (
    <Card
      role="region"
      aria-label="Plagiarism analysis"
      className="mt-7 gap-0 p-4 sm:p-6"
    >
      ...
      <p className="mt-1 max-w-3xl text-sm leading-relaxed text-muted-foreground">
        Exact and modified-copy detection in this phase. Matching content is
        reported — never an accusation of who copied from whom.
      </p>

      {report !== null && sessionStatus !== "locked" ? (
        <section
          aria-labelledby="session-certification-title"
          className="mt-5 max-w-3xl rounded-lg border border-border/80 bg-muted/30 p-4 sm:p-5"
        >
          <p className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">
            Session certification · Step 1 of 2
          </p>
          <h3
            id="session-certification-title"
            className="mt-1 text-base font-semibold tracking-tight"
          >
            Freeze the reviewed evidence
          </h3>
          <p className="mt-1 max-w-2xl text-sm leading-relaxed text-muted-foreground">
            Locking signs the current inputs and comparison corpus, then
            prevents edits, re-analysis, and library removal. There is no unlock
            path. The private key stays in the operating-system credential
            store.
          </p>
          {lockError ? (
          ...
          <Button
            ...
            disabled={locking || analyzing}
          >
            {locking
              ? "Signing and locking…"
              : "Lock session for certification"}
          </Button>
        </section>
      ) : null}
      ...
      {sessionStatus === "locked" ? (
        <section
          ...
          className="mt-5 max-w-3xl rounded-lg border border-emerald-900/15 bg-emerald-50/50 px-4 py-4 sm:px-5"
        >
          <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
            <h3
              id="locked-session-title"
              className="text-base font-semibold tracking-tight"
            >
              Session locked · inputs frozen
            </h3>
            {lockQuery.data ? (
              <time
                className="text-xs text-muted-foreground"
                dateTime={lockQuery.data.locked_at}
              >
                {new Date(lockQuery.data.locked_at).toLocaleString()}
              </time>
            ) : null}
          </div>
          {lockQuery.isPending ? (
            <p className="mt-2 text-sm text-muted-foreground">
              Verifying the signed lock…
            </p>
          ) : null}
          {lockQuery.isError ? (
            <p role="alert" className="mt-2 text-sm text-destructive">
              {asSessionsError(lockQuery.error).message}
            </p>
          ) : null}
          {lockQuery.data ? (
            <>
              ...
              <code className="mt-1 block break-all font-mono text-xs">
                {lockQuery.data.manifest_sha256}
              </code>
              <p className="mt-3 text-sm text-muted-foreground">
                Signer key ID
              </p>
              <code className="mt-1 block break-all font-mono text-xs">
                {lockQuery.data.signing_key_id}
              </code>
              <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
                This device key identifies the signing key, not a person.
                Confirm its fingerprint with the teacher through a trusted
                channel.
              </p>
            </>
          ) : null}
      ...
      {loadingSaved ? (
        <div className="mt-3">
          <LoadingState label="Loading saved analysis…" />
        </div>
        ...
        <div className="mt-3">
          <LoadingState label="Checking the selected comparison corpus…" />
        </div>
      ) : submissionCount === 1 && selectedLibrariesQuery.isError ? (
        <Alert variant="destructive" className="mt-3">
          <AlertDescription>
            Could not check the selected comparison libraries:{" "}
            {asSessionsError(selectedLibrariesQuery.error).message}
          </AlertDescription>
        </Alert>
      ) : !canAnalyze ? (
        <div className="mt-3">
          <EmptyState
            title={
              submissionCount === 0
                ? "Add a submission to begin"
                : "Choose a comparison source"
            }
            description={
              submissionCount === 0
            ...
            action={
              submissionCount > 0 ? (
                <Button
                  ...
                  onClick={() =>
                    document
                      .getElementById("session-reference-libraries")
        ...
        <div className="mt-5 space-y-7">
          <div className="flex items-center gap-2">
            {sessionStatus === "locked" ? (
              <p className="text-sm text-muted-foreground">
                Analysis is frozen with the signed session lock.
              </p>
            ) : (
              <Button
                variant="outline"
          ...
          {sessionStatus !== "locked" ? (
            <section
              ...
              className="max-w-4xl rounded-lg border border-primary/15 bg-gradient-to-br from-primary/[0.055] to-card p-4 shadow-sm shadow-primary/[0.035] sm:p-5"
            >
              ...
              >
                Review against your selected sources
              </h3>
              <p className="mt-1 max-w-3xl text-sm leading-relaxed text-muted-foreground">
                This report shows matching passages only in the comparison
                corpus listed here. It is not a universal plagiarism check and
                does not make a misconduct decision.
              </p>
              <div
                className="mt-3 flex flex-wrap gap-2"
                aria-label="Self-check comparison corpus"
              >
                <span className="rounded-md border bg-background/70 px-2.5 py-1.5 text-xs text-muted-foreground">
                  {report.pairs.length > 0
                    ? `${report.pairs.length} current-session comparisons`
                    : "No other current submissions"}
                </span>
                {report.compared_libraries.map((library) => (
                  <span
                    key={library.id}
                    className="rounded-md border bg-background/70 px-2.5 py-1.5 text-xs text-muted-foreground"
                  >
                    {library.name} · {library.source_count} archived{" "}
                    {library.source_count === 1 ? "submission" : "submissions"}
                  </span>
                ))}
                ...
                report.pairs.length === 0 ? (
                  <span className="rounded-md border border-dashed px-2.5 py-1.5 text-xs text-muted-foreground">
                    No comparison sources were selected
                  </span>
                ) : null}
              </div>
            </section>
          ...
          {sessionStatus === "locked" && lockQuery.data ? (
            <section
              ...
              className="max-w-3xl rounded-lg border border-border/80 bg-muted/25 p-4 sm:p-5"
            >
              <h3
              ...
              <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
                Choose the JSON file exported beside the PDF. Verification
                checks the report contents, reviewer footer, lock digest, key
                ID, and Ed25519 signature.
              </p>
              <div className="mt-3 flex flex-col items-start gap-3 sm:flex-row sm:items-center">
                <Input
                  ...
                  className="h-auto max-w-full border-dashed bg-background p-2 text-sm file:mr-3 file:rounded-md file:border file:bg-muted file:px-3 file:py-2 file:text-sm file:font-medium hover:file:bg-muted/70"
                />
                <Button
                  variant="outline"
                  onClick={() => void onVerifyReport()}
              ...
              {verificationNote ? (
                <p
                  role="status"
                  className="mt-3 border-l-2 border-foreground/30 pl-3 text-sm leading-relaxed"
                >
                  {verificationNote}
                </p>
              ) : null}
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Each cell uses the row student’s eligible words, so the reverse
            ...
            <div className="mt-3 overflow-hidden rounded-lg border border-border/80">
              <Table className="min-w-[320px] text-sm">
                ...
                <TableBody>
                  {students.map((row) => (
                    <TableRow key={row.id}>
                      <TableHead scope="row">{row.display_name}</TableHead>
                      {students.map((col) => {
                        if (row.id === col.id) {
                          return (
                            <TableCell
                              ...
                              aria-label={`${row.display_name} vs self`}
                            >
                              —
                            </TableCell>
                          );
                        ...
                        const pair = report.pairs.find(
                          (p) =>
                            (p.a_student_id === row.id &&
                              p.b_student_id === col.id) ||
                            (p.a_student_id === col.id &&
                              p.b_student_id === row.id),
                        );
                        ...
                        return (
                          <TableCell key={col.id}>
                            <Button
                              ...
                              className={cn(
                                "h-auto rounded-md px-2 py-1 font-mono text-xs",
                                hasPassages
          ...
          <div>
            ...
            <div className="mt-2 flex items-start gap-2 text-sm">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setAnonymizeReports(checked === true)
                }
                className="mt-0.5"
              />
              <Label htmlFor="anonymize-reports" className="leading-relaxed">
            ...
            <ul className="mt-3 grid gap-3">
              {report.per_student.map((row) => (
                <li
                  key={row.student_id}
                  className="rounded-md border border-border/70 bg-background p-3.5 sm:p-4"
                >
                  <div className="flex items-center justify-between text-sm">
                    ...
                    {row.coverage === null || row.coverage === undefined ? (
                      ...
                      <span className="font-mono text-xs text-muted-foreground">
                        {formatPct(row.coverage)} · {row.matched_tokens}/
                        {row.eligible_tokens} eligible words
                      </span>
                    )}
                  </div>
                  {row.coverage === null ||
                  row.coverage === undefined ? null : (
                    <p className="text-xs text-muted-foreground">
                      Exact {formatOptionalPct(row.exact_coverage)} · Modified{" "}
                      {formatOptionalPct(row.modified_coverage)}
                  ...
                  row.coverage === undefined ? null : (
                    <ProgressIndicator
                      value={row.coverage}
                      label={`Overlap for ${nameById.get(row.student_id) ?? "student"}`}
                    />
                  )}
                  {sessionStatus === "locked" ? (
                    <Button
                    ...
                    >
                      {reportingStudentId === row.student_id
                        ? "Signing report…"
                        : "Generate certified report for " +
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      ...
                      >
                        {selfCheckStudentId === row.student_id
                          ? "Preparing PDF…"
                          : "Download Self Check PDF"}
                      </Button>
          ...
          {report.compared_libraries.length > 0 ? (
            <section aria-label="Historical reference comparisons">
              ...
              <ul className="mt-2 space-y-1 text-sm">
                {report.compared_libraries.map((library) => (
                  <li key={library.id}>
                    {library.name} · {library.source_count} historical
                    submissions
                    <span className="text-muted-foreground">
                      {" "}
                      · from {library.source_session_name}
                    </span>
                  </li>
                ))}
              </ul>
              {report.historical_matches.length === 0 ? (
                <p className="mt-3 text-sm text-muted-foreground">
                  No matching passages were found in the selected historical
                  libraries.
                </p>
              ) : (
                <ol className="mt-3 space-y-4">
                  {report.historical_matches.map((match) => (
                    <HistoricalMatchCard
                      ...
                      match={match}
                      studentName={
                        nameById.get(match.student_id) ?? "Unknown student"
                      }
          ...
          {selectedPair ? (
            <div
              ...
              className="rounded-lg border border-border/80 bg-muted/20 p-4 sm:p-5"
            >
              <h3 className="text-sm font-semibold">
                {nameById.get(selectedPair.a_student_id)} vs{" "}
                {nameById.get(selectedPair.b_student_id)} —{" "}
                {selectedPair.coverage_a === null ||
                selectedPair.coverage_a === undefined
                  ? "not assessable"
                  : formatPct(selectedPair.coverage_a)}{" "}
                /{" "}
                {selectedPair.coverage_b === null ||
                selectedPair.coverage_b === undefined
                  ? "not assessable"
                  : formatPct(selectedPair.coverage_b)}
              </h3>
              <p className="mt-1 text-xs text-muted-foreground">
                {nameById.get(selectedPair.a_student_id)}: exact{" "}
                {formatOptionalPct(
                  cellCoverageByKind(
                    selectedPair,
                    selectedPair.a_student_id,
                    "exact",
                  ),
                )}
                , modified{" "}
                ...
                · {nameById.get(selectedPair.b_student_id)}: exact{" "}
                {formatOptionalPct(
                  cellCoverageByKind(
                    selectedPair,
                    selectedPair.b_student_id,
                    "exact",
                  ),
                )}
                , modified{" "}
                ...
                . Type values are subsets; combined coverage uses unique spans.
              </p>
                ...
                <ol className="mt-3 space-y-4">
                  {selectedPair.passages.map((passage, i) => (
                    <PassageCard
                      ...
                      passage={passage}
                      aName={
                        nameById.get(selectedPair.a_student_id) ?? "Student A"
                      }
                      bName={
                        nameById.get(selectedPair.b_student_id) ?? "Student B"
                      }
                      ...
                      return (
                        ...
                        >
                          <p className="text-xs font-medium text-muted-foreground">
                            ...
                            {nameById.get(studentId)} · {item.tokens} words not
                            counted
                          </p>
                          <div className="mt-1">
                            <HighlightedText
  ...
  return (
    <li className="rounded-lg border p-4">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        ...
        {match.reference_filename ?? "an archived submission"} · current-text
        coverage {formatOptionalPct(match.coverage_current)}
      </p>
      <p className="mt-1 text-xs text-muted-foreground">
        Exact {formatOptionalPct(match.exact_coverage_current)} · Modified{" "}
        {formatOptionalPct(match.modified_coverage_current)}
      </p>
  ...
  return (
    ...
    >
      ...
      {progress.total_pairs > 0 ? (
        <p className="mt-1 text-xs text-muted-foreground">
          {progress.completed_pairs}/{progress.total_pairs} pairs ·{" "}
          {progress.cached_pairs} unchanged raw pair results reused
        </p>
      ) : null}
    </div>
  );

...
describe("analysis API", () => {
  it("calls analyze with the session id", async () => {
    ...
    expect(mock).toHaveBeenCalledWith("analyze_session", {
      sessionId: "sess-1",
    });
  });
  ...
  it("rejects malformed reports as protocol errors", async () => {
    ...
    await expect(analyzeSession("sess-1")).rejects.toMatchObject({
      code: "protocol",
    });
  });
  ...
  it("returns the chosen save path, reports cancellation, and rejects malformed results", async () => {
    const invoke = vi.fn(async () => "/tmp/provenance-report.pdf");
    setInvokeImpl(invoke);
    await expect(
      saveReportFiles([37, 80, 68, 70, 45], "provenance-report.pdf"),
    ).resolves.toBe("/tmp/provenance-report.pdf");
    expect(invoke).toHaveBeenCalledWith("save_report_files", {
      pdfBytes: [37, 80, 68, 70, 45],
      suggestedFileName: "provenance-report.pdf",
      companionJson: null,
    });

    setInvokeImpl(vi.fn(async () => null));
    await expect(
      saveReportFiles([37, 80, 68, 70, 45], "provenance-report.pdf"),
    ).resolves.toBeNull();

    setInvokeImpl(vi.fn(async () => ({ path: "/tmp/provenance-report.pdf" })));
    await expect(
      saveReportFiles([37, 80, 68, 70, 45], "provenance-report.pdf"),
    ).rejects.toMatchObject({ code: "protocol" });
  });
  ...
  it("requires a complete signed export and verifies certificates through Tauri", async () => {
    ...
    const invoke = vi.fn(async (cmd: string) => {
      if (cmd === "generate_certified_student_report") {
        return {
          ...
          signed_report_json: signedReportJson,
        };
      }
      return {
        status: "verified",
    ...
    setInvokeImpl(invoke);
    await expect(
      generateCertifiedStudentReport("sess-1", "student-1", true),
    ).resolves.toEqual({
      pdfBytes: [37, 80, 68, 70, 45],
      signedReportJson,
    });
    ...
    await expect(
      verifyCertifiedStudentReport(signedReportJson),
    ...
    await expect(
      generateCertifiedStudentReport("sess-1", "student-1", false),
    ).rejects.toMatchObject({ code: "protocol" });
  });
...
describe("analysis section", () => {
  it("requires an explicit comparison source for a single submission", async () => {
    ...
    expect(
      await screen.findByRole("heading", {
        name: /choose a comparison source/i,
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /choose a reference library/i }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /analyze session/i }),
    ).not.toBeInTheDocument();
  });

  it("saves a clearly uncertified self-check to the selected location", async () => {
    ...
    const invokeSpy = vi.fn((cmd: string, args?: Record<string, unknown>) => {
      ...
      if (cmd === "save_report_files")
        return Promise.resolve("/tmp/provenance-st-a-self-check.pdf");
    ...
    await screen.findByRole("heading", { name: /history test/i });
    await user.click(
      await screen.findByRole("button", { name: /analyze session/i }),
    );
    await screen.findByRole("heading", {
      name: /review against your selected sources/i,
    });
    expect(
      screen.getByText(/course archive · 1 archived submission/i),
    ).toBeInTheDocument();
    expect(
      screen.getAllByText(/not teacher certified/i).length,
    ).toBeGreaterThan(0);
    await user.click(
      screen.getByRole("button", { name: /download self check pdf/i }),
    );
    await screen.findByText(
      /self check pdf saved to \/tmp\/provenance-st-a-self-check\.pdf/i,
    );
    ...
    expect(invokeSpy).toHaveBeenCalledWith("save_report_files", {
      pdfBytes: [37, 80, 68, 70, 45],
      suggestedFileName: "provenance-st-a-self-check.pdf",
      companionJson: null,
    });
  });
  ...
  it("runs analysis and shows the pairwise matrix", async () => {
    ...
    expect(
      await screen.findByText(
        /each cell uses the row student’s eligible words/i,
      ),
    ).toBeInTheDocument();
    const cell = await screen.findByRole("button", {
      name: /amit vs priya: \d+% overlap/i,
    });
    expect(cell.textContent).not.toBe("0%");
    // Unrelated pair stays clean.
    expect(
      screen.getByRole("button", { name: /amit vs chen: 0% overlap/i }),
    ).toBeInTheDocument();
  });
  ...
  it("shows live monotonic progress from the analysis event channel", async () => {
    ...
    setInvokeImpl((cmd, args) => {
      if (cmd === "analyze_session") {
        return new Promise((_, reject) => {
          setTimeout(
            () => reject({ code: "cancelled", message: "Test stopped." }),
            80,
          );
        });
      }
    ...
    expect(
      within(progress).getByText(/finding exact matches/i),
    ).toBeInTheDocument();
    expect(progress.querySelector("progress")).toHaveValue(0.5);
    expect(
      within(progress).getByText(
        /2\/3 pairs · 1 unchanged raw pair results reused/i,
      ),
    ).toBeInTheDocument();
  ...
  it("opens pair detail with highlighted passages", async () => {
    ...
    expect(
      within(detail).getByText(/passage 1 · \d+ matching words/i),
    ).toBeInTheDocument();
    expect(
      within(detail).getByText(
        /type values are subsets; combined coverage uses unique spans/i,
      ),
    ).toBeInTheDocument();
    const marks = within(detail).getAllByText(
      /rapid expansion of railway networks/i,
    );
  ...
  it("shows zero-match detail for clean pairs", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: /amit vs chen: 0% overlap/i }),
    );
    expect(
      await screen.findByText(/no matching passages detected/i),
    ).toBeInTheDocument();
  });
  ...
  it("locks a reviewed session and saves the signed PDF plus verification JSON together", async () => {
    const user = userEvent.setup();
    const { invoke } = createFakeSessions(seedThreeStudents());
    const invokeSpy = vi.fn((cmd: string, args?: Record<string, unknown>) => {
      if (cmd === "generate_certified_student_report") {
      ...
      if (cmd === "save_report_files")
        return Promise.resolve("/tmp/provenance-st-a-certified-report.pdf");
      return invoke(cmd, args);
    });
    setInvokeImpl(invokeSpy);
    renderRouteAt("/sessions/sess-5");
    ...
    await screen.findByRole("heading", {
      name: /freeze the reviewed evidence/i,
    });
    expect(
      screen.getAllByRole("button", { name: /download self check pdf/i }),
    ).toHaveLength(3);
    await user.click(
      await screen.findByRole("button", {
        name: /lock session for certification/i,
      }),
    );
    const reportButton = await screen.findByRole("button", {
      name: /generate certified report for amit/i,
    });
    await vi.waitFor(() => expect(reportButton).toBeEnabled());
    await user.click(
      screen.getByRole("checkbox", { name: /anonymize names and filenames/i }),
    );
    ...
    expect(invokeSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        pdfBytes: [37, 80, 68, 70, 45],
        suggestedFileName: "provenance-st-a-certified-report.pdf",
        companionJson: expect.stringContaining('"schema_version":1'),
      }),
    );
    await screen.findByText(
      /certified pdf saved to \/tmp\/provenance-st-a-certified-report\.pdf/i,
    );
  });
